 ...
 */
class DefaultSimpleBridgeElem(val backingElem: simple.Elem) extends AnyVal with SimpleBridgeElem {

  final type BackingElem = simple.Elem

  final type SelfType = DefaultSimpleBridgeElem

  final def findAllChildElems: immutable.IndexedSeq[SelfType] =
    backingElem.findAllChildElems.map(e => new DefaultSimpleBridgeElem(e))
  ...
  final def findChildElemByPathEntry(entry: Path.Entry): Option[SelfType] =
    backingElem.findChildElemByPathEntry(entry).map(e => new DefaultSimpleBridgeElem(e))

  final def toElem: simple.Elem = backingElem
}

object DefaultSimpleBridgeElem {

  def wrap(elem: simple.Elem): DefaultSimpleBridgeElem = new DefaultSimpleBridgeElem(elem)
}
