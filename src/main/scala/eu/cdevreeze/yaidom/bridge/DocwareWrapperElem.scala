...
/**
 * This object is like a `DocawareBridgeElem`, but offering the `ScopedElemLike` and `IsNavigable` query API.
 * It is itself also a `DocawareBridgeElem`, and wraps a `DocawareBridgeElem`.
 *
 * Use this object when you want a `DocawareBridgeElem`, but at the same time want to use the yaidom query API.
 *
 * @author Chris de Vreeze
 */
final class DocawareWrapperElem(val bridge: DocawareBridgeElem) extends DocawareBridgeElem with ScopedElemLike[DocawareWrapperElem] with IsNavigable[DocawareWrapperElem] { self: DocawareWrapperElem =>

  final override type BackingElem = bridge.BackingElem

  final override type SelfType = DocawareWrapperElem

  final override type UnwrappedBackingElem = bridge.UnwrappedBackingElem

  final def backingElem: BackingElem = bridge.backingElem

  final def findAllChildElems: immutable.IndexedSeq[DocawareWrapperElem] = {
    bridge.findAllChildElems.map(e => new DocawareWrapperElem(e))
  }
